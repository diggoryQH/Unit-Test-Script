...
class SendMailServiceTest {
     ...
     * Đầu vào : mailInfo hợp lệ (from, to, subject, body, attachments=null) — từ
     * setUp()
    ...
    void TC_SML_01() throws MessagingException, IOException {
     ...
     * File đính kèm không tồn tại thực sự → mong đợi exception từ
     * FileSystemResource,
     ...
     */
    @Test // [Branch Coverage] Nhánh: attachments != null → service thực hiện thêm file
          // đính kèm
    void TC_SML_02() throws MessagingException, IOException {
     ...
     * Đầu vào : mailInfo với body = "<html><body>
     * <h1>Invoice</h1>
     * <p>
     * Total: 1,000,000 VND
     * </p>
     * </body></html>"
     * Hành vi GS: javaMailSender.createMimeMessage() → mimeMessage
     * Kết quả KV: send(mimeMessage) được gọi 1 lần (HTML body được xử lý bình
     * thường)
     */
    @Test // [Happy Path] Gửi email với body HTML đầy đủ — helper.setText(body, true)
          // không lỗi
    void TC_SML_03() throws MessagingException, IOException {
    ...
    void TC_SML_04() throws MessagingException, IOException {
    ...
    /**
     * TC_SML_05
     * Mục tiêu : Gửi email đặt lại mật khẩu — mô phỏng use case thực tế của hệ
     * thống.
     ...
     * body="
     * <p>
     * Click <a href='...?token=abc123'>here</a>...
     * </p>
     * ")
     * Hành vi GS: javaMailSender.createMimeMessage() → mimeMessage
     * Kết quả KV: send(mimeMessage) được gọi 1 lần
     */
    @Test // [Happy Path] Gửi email reset password — use case thực tế, body chứa token
          // link
    void TC_SML_05() throws MessagingException, IOException {
        ...
        resetMail.setBody(
                "<p>Click <a href='http://localhost:4200/reset-password?token=abc123'>here</a> to reset password</p>");
    ...
    /**
     * TC_SML_06
     * Mục tiêu : Gửi email xác nhận đơn hàng — mô phỏng use case gửi mail sau khi
     * checkout.
    ...
    void TC_SML_06() throws MessagingException, IOException {
    ...
    void TC_SML_07() throws MessagingException, IOException {
    ...
    /**
     * TC_SML_08
     * Mục tiêu : SMTP server gặp lỗi khi createMimeMessage() — mô phỏng lỗi kết nối
     * SMTP.
     * Đầu vào : mailInfo hợp lệ — createMimeMessage() ném RuntimeException("SMTP
     * Error")
     * Hành vi GS: javaMailSender.createMimeMessage() → ném RuntimeException("SMTP
     * Error")
     * Kết quả KV: Exception được ném, message = "SMTP Error"
     */
    @Test // [Branch Coverage] SMTP lỗi — createMimeMessage() ném RuntimeException
    void TC_SML_08() throws MessagingException, IOException {
    ...
    /**
     * TC_SML_09
     * Mục tiêu : Subject chứa ký tự đặc biệt và emoji — xác nhận UTF-8 encoding
     * hoạt động đúng.
    ...
    void TC_SML_09() throws MessagingException, IOException {
    ...
    /**
     * TC_SML_10
     * Mục tiêu : Body chứa nội dung tiếng Việt có dấu — xác nhận UTF-8 encoding bảo
     * toàn nội dung.
     * Đầu vào : MailInfo với body = "
     ...
     * Kết quả KV: send(mimeMessage) gọi 1 lần, không ném exception về encoding
     */
    @Test // [Edge Case] Body chứa tiếng Việt có dấu — kiểm tra UTF-8 không làm hỏng nội
          // dung
    void TC_SML_10() throws MessagingException, IOException {
    ...
    /**
     * TC_SML_11
     * Mục tiêu : Body email có kích thước rất lớn (1000 đoạn HTML) — kiểm tra không
     * timeout/tràn bộ nhớ.
     * Đầu vào : mailInfo với body = chuỗi 1000 thẻ
     * <p>
     * (khoảng 35KB)
     * Hành vi GS: javaMailSender.createMimeMessage() → mimeMessage
     * Kết quả KV: send(mimeMessage) gọi 1 lần, service không crash với body lớn
     */
    @Test // [Edge Case] Body rất lớn (~35KB, 1000 thẻ HTML) — service không crash hay
          // timeout
    void TC_SML_11() throws MessagingException, IOException {
    ...
    // ==========================================
    // MODULE: THÊM MAIL VÀO HÀNG ĐỢI — queue(MailInfo) &
     ...
     * Đầu vào : MailInfo(to="recipient@example.com", subject="Welcome", body="Hello
     * User")
     * Kết quả KV: javaMailSender.send() KHÔNG được gọi (mail chỉ nằm trong list
     * chờ)
     */
    ...
    /**
     * TC_SML_13
     * Mục tiêu : Thêm nhiều mail vào hàng đợi liên tiếp bằng overload
     * queue(String,String,String).
     * Xác nhận không mail nào bị gửi ngay, tất cả chờ trong list.
     * Đầu vào : 2 lần gọi queue(to, subject, body) với thông tin khác nhau
     * Kết quả KV: javaMailSender.send() KHÔNG được gọi (cả 2 mail đều nằm trong
     * list chờ)
     */
    @Test // [Happy Path] queue(String,String,String) overload — nhiều mail vào list,
          // không gửi ngay
    void TC_SML_13() {
     ...
     * Xác nhận queue() không thực thi send() ngay cả khi from/to/subject/body đầy
     * đủ.
     * Đầu vào : MailInfo đầy đủ: from="sender@nongsan.com",
     * to="receiver@example.com",
     * subject="Promotional Email", body="
     * <h2>Special Offer</h2>"
     ...
     * Kết quả KV: javaMailSender.send() KHÔNG được gọi
     */
    @Test // [Happy Path] queue(to,subject,body) 3 tham số — wrapper gọi queue(MailInfo),
          // không gửi ngay
    void TC_SML_15() {
