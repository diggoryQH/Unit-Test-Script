...
class CheckoutApiTest {
        ...
        /**
         * TC_CKO_01
         * Mục tiêu : Đặt hàng thành công — email tồn tại, cartId hợp lệ, giỏ có 1 sản
         * phẩm.
         * Đầu vào : email = "user@gmail.com", CartRequest(cartId=1, amount=40000,
         * shippingFee=15000)
        ...
        void TC_CKO_01() throws Exception {
        ...
        /**
         * TC_CKO_02
         * Mục tiêu : Đặt hàng thất bại khi email người dùng không tồn tại trong hệ
         * thống.
         * Đầu vào : email = "ghost@gmail.com" (không có trong DB), CartRequest hợp lệ
         * Hành vi GS: cartRepository.findById → Optional(mockCart) [được gọi trước khi
         * check email]
        ...
        void TC_CKO_02() throws Exception {
         ...
         * Hành vi GS: cartRepository.findById → Optional(mockCart) [code gọi .get()
         * không an toàn]
        ...
        void TC_CKO_03() throws Exception {
                String email = "user@gmail.com";
        ...
        /**
         * TC_CKO_04
         * Mục tiêu : Đặt hàng với nhiều sản phẩm khác nhau — kiểm tra đúng số lần gọi
         * save.
         ...
         */
        @Test // [Edge Case] Giỏ có nhiều sản phẩm → lưu đúng số OrderDetail và xóa đúng số
              // CartDetail
        void TC_CKO_04() throws Exception {
                String email = "user@gmail.com";
                Long cartId = 1L;
        ...
        /**
         * TC_CKO_05
         * Mục tiêu : Đặt hàng với giỏ hàng rỗng (0 CartDetail) — xác nhận hành vi thực
         * tế của hệ thống.
         ...
         * sendMailUtil.sendMailOrder được gọi 1 lần
         */
        @Test // [Edge Case - BUG DETECT] Giỏ hàng rỗng → Mong đợi 400 Bad Request
        void TC_CKO_05() throws Exception {
                String email = "user@gmail.com";
                Long cartId = 1L;
                ...
                // Khi giỏ rỗng, hệ thống KHÔNG ĐƯỢC PHÉP lưu đơn hàng hay gửi mail
                Mockito.verify(orderRepository, Mockito.never()).save(any(Order.class));
