...
class VnPayControllerTest {
         ...
         * Verify : vnPayService.createOrder được gọi với amount=100000L
         * (amount.longValue())
         */
        @Test // [Happy Path] Tạo URL thanh toán thành công — amount hợp lệ
        void TC_VNP_01() throws Exception {
         ...
         */
        @Test // [Edge Case] Amount = 0 — Controller không validate, vẫn chuyển tiếp cho
              // Service
        void TC_VNP_02() throws Exception {
        ...
        /**
         * TC_VNP_03
         * Mục tiêu : Thanh toán thành công — chữ ký hợp lệ, vnp_TransactionStatus =
         * "00".
        ...
        void TC_VNP_03() throws Exception {
        ...
        /**
         * TC_VNP_04
         * Mục tiêu : Thanh toán thất bại — chữ ký hợp lệ nhưng user hủy giao dịch
         * (ResponseCode="24").
        ...
        void TC_VNP_04() throws Exception {
        ...
        /**
         * TC_VNP_05
         * Mục tiêu : Sai chữ ký bảo mật — request có thể bị giả mạo
         * (INVALID_SIGNATURE).
        ...
        void TC_VNP_05() throws Exception {
                Mockito.when(vnPayService.orderReturn(any())).thenReturn(-1);
        ...
        /**
         * TC_VNP_06
         * Mục tiêu : Callback thiếu tham số vnp_Amount — kiểm tra Controller không
         * crash.
         * Controller dùng request.getParameter() → trả về null nếu thiếu tham số.
         * Hệ thống vẫn xử lý dựa vào kết quả của orderReturn.
         * Đầu vào : Chỉ truyền vnp_OrderInfo, vnp_ResponseCode, vnp_SecureHash (thiếu
         * vnp_Amount)
         ...
         * Verify : vnPayService.orderReturn được gọi đúng 1 lần, hệ thống không ném
         * exception
         */
        @Test // [Edge Case] Thiếu tham số vnp_Amount — hệ thống không crash, vẫn trả kết quả
        void TC_VNP_06() throws Exception {
                Mockito.when(vnPayService.orderReturn(any())).thenReturn(1);
